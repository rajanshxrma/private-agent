...
4. **A macOS 27 regression found 2026-07-09, fixed and re-verified closed 2026-07-10 -- see `voice.py`'s module docstring for the full diagnostic trail.** `listen()`'s record-then-transcribe flow originally reliably returned empty via a silent 15s stall, even though recording and transcription were each independently confirmed healthy in isolation (a fresh recording measured a strong 0.54+ peak amplitude, and transcribing that exact file in a separate process succeeded perfectly, verbatim). Root cause: some form of per-process resource contention between `AVAudioEngine`'s input tap and `SFSpeechRecognizer`'s on-device path on this OS -- confirmed structural (an explicit `AVAudioSession` teardown did not fix it) rather than a flush-timing gap (a settle delay didn't fix it either). Fix: transcription now runs in its own freshly-spawned subprocess, mirroring the already-proven "a fresh process transcribes fine" case. Re-verification (same-day follow-up): the subprocess path was tested head-to-head against direct in-process transcription on the *same* recordings -- identical, verbatim-correct output every trial (~1.3s subprocess), zero stalls across dozens of calls. The head-to-head also pinned down a separate recognizer-level behavior that had muddied earlier measurements: under elevated ambient noise, a short (~1.5s) utterance inside the full 5s fixed window deterministically returns empty *in both paths* -- while the same phrase in a shorter window, or a longer phrase filling the 5s window, transcribes perfectly. That's a window-proportion effect of fixed-duration recording (see Limitations), not the stall and not the fix.

**Real, measured reliability -- stated honestly, not oversold (numbers below predate the macOS 27 regression above; on that OS post-fix, spot-checks confirm the transcription path itself is intact -- see finding #4 -- with the added caveat that short utterances in a noisy room now have a known deterministic empty-result mode):** across ~25 real test trials (synthesized speech via macOS's `say`, picked up by the actual microphone, transcribed by the actual on-device recognizer), single-attempt success ranged 67-87% depending on the batch. Failures split into two different kinds: genuine silence/nothing-heard (which a retry reliably fixes) and outright misrecognition -- the recognizer confidently transcribing the wrong short word ("Sachin" for "testing", "Siri" for "draft an email"). `listen()` retries automatically up to 3 attempts total on an empty result, which pushes "at least one attempt returns *something*" past 99% at the measured per-attempt rate -- but a retry can't fix misrecognition, since a non-empty (if wrong) result stops the retry loop. This is a real, honest limitation of speech recognition technology generally (no STT system, including Apple's own Siri/Dictation, is 100%), likely made *worse* here than real usage would show: this session's own testing is a Mac's speaker bouncing synthesized audio back into its own microphone, which is measurably noisier than a human voice spoken directly at the laptop from normal distance. Real human-voice reliability is expected to be higher than these numbers, but hasn't been separately validated in this session -- worth a real spot-check before leaning on this for anything time-sensitive.
...
- **On macOS 27 (beta): voice input's silent-stall bug is fixed and re-verified closed.** See finding #4 in the Voice front-end section above for the full status -- the original total failure (silent 15s stall, zero successes) is resolved via subprocess-isolated transcription, verified head-to-head against direct transcription on identical recordings. Everything else in this repo (MLX/AFM backends, tools, router) is confirmed working normally on this same OS via the stability checkpoint this finding came from.
- **Short utterances in a noisy room can deterministically transcribe as empty.** Matrix-tested on macOS 27: with ambient noise a few times above the quiet floor, a ~1.5s utterance followed by several seconds of room noise inside the fixed 5s window returns empty in every transcription path, while longer utterances filling the window transcribe fine under the same conditions. `listen()`'s retry covers the occasional miss; a persistently empty result in a noisy environment is this, not a bug in the pipeline. Speak a full phrase, or speak closer to the mic.
