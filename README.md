...
4. **A known, currently UNRESOLVED regression found 2026-07-09 during a stability checkpoint after upgrading this machine to macOS 27 (beta).** `listen()`'s record-then-transcribe flow now reliably returns empty (4 consecutive full test failures, 12 total real attempts, zero successes) -- but each half is independently confirmed healthy in isolation on the same machine at the same time: a fresh recording measured a strong 0.54 peak amplitude (well within the real-speech range above), and transcribing that exact file in a separate later process succeeded perfectly, verbatim. This looks like bug #2 above (a record/transcribe race within one process) recurring in a form the existing `.close()` fix no longer fully covers on this OS -- but adding an explicit settle delay (tried 0.2s and 1s) did **not** fix it, ruling out a simple flush-timing gap and pointing at something more structural, most likely an `AVAudioEngine`/`AVAudioSession` resource-release interaction with the Speech framework that changed on macOS 27. Root cause not found; see `voice.py`'s module docstring for the full diagnostic trail. Confirmed real, not pre-existing: this exact flow was verified working end-to-end when the module was first built, before this machine's OS upgrade. Whether this is a macOS 27 beta issue that resolves itself in a later beta seed, or a real bug worth reporting to Apple via Feedback Assistant, is still open.

**Real, measured reliability -- stated honestly, not oversold (numbers below predate the macOS 27 regression above; not yet re-measured on that OS since `listen()` currently doesn't work there at all):** across ~25 real test trials (synthesized speech via macOS's `say`, picked up by the actual microphone, transcribed by the actual on-device recognizer), single-attempt success ranged 67-87% depending on the batch. Failures split into two different kinds: genuine silence/nothing-heard (which a retry reliably fixes) and outright misrecognition -- the recognizer confidently transcribing the wrong short word ("Sachin" for "testing", "Siri" for "draft an email"). `listen()` retries automatically up to 3 attempts total on an empty result, which pushes "at least one attempt returns *something*" past 99% at the measured per-attempt rate -- but a retry can't fix misrecognition, since a non-empty (if wrong) result stops the retry loop. This is a real, honest limitation of speech recognition technology generally (no STT system, including Apple's own Siri/Dictation, is 100%), likely made *worse* here than real usage would show: this session's own testing is a Mac's speaker bouncing synthesized audio back into its own microphone, which is measurably noisier than a human voice spoken directly at the laptop from normal distance. Real human-voice reliability is expected to be higher than these numbers, but hasn't been separately validated in this session -- worth a real spot-check before leaning on this for anything time-sensitive.
...
- **On macOS 27 (beta): voice input currently doesn't work at all.** A real, unresolved regression -- see finding #4 in the Voice front-end section above. Recording and transcription are each independently confirmed healthy on this OS; only the combined immediate record-then-transcribe flow inside `listen()` is affected. Everything else in this repo (MLX/AFM backends, tools, router) is confirmed working normally on this same OS via the stability checkpoint this finding came from.
